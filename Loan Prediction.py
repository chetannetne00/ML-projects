...
def bhattacharya_distance(p,q):
    lft_trm_1 = math.pow(np.std(p),2)/math.pow(np.std(q),2)
    lft_trm_2 = math.pow(np.std(q),2)/math.pow(np.std(p),2)
    lft_trm = 1/4*np.log(1/4*(lft_trm_1+lft_trm_2+2))
    rgt_trm = 1/4*((math.pow(np.mean(p)-np.mean(q),2))/(math.pow(np.std(p),2)+math.pow(np.std(q),2)))
    distance=lft_trm + rgt_trm
...
a= bhattacharya_distance(comb_data['ApplicantIncome'],comb_data['LoanAmount'])
b= math.sqrt(np.std(comb_data['ApplicantIncome']))/math.sqrt(np.std(comb_data['LoanAmount']))
a
...
def bhattacharya(P,Q):
    #mu1_minus_mu2 = np.mean(P) - np.mean(Q)
    return np.mean(P) - np.mean(Q)
...
comb_data.describe()
P=['ApplicantIncome','CoapplicantIncome',]
Q=['LoanAmount']
bhattacharya(comb_data[P],comb_data[Q])
